...
class TabelaOlcumGUI:
    def __init__(self, root):
        ...
        self.canvas.bind("<Motion>", self.show_zoom)
    ...
    def show_zoom(self, event):
        ...
        real_x = int((x - self.img_offset_x) / self.scale_ratio)
        real_y = int((y - self.img_offset_y) / self.scale_ratio)
        ...
        x1 = max(0, real_x - zoom_size)
        y1 = max(0, real_y - zoom_size)
        x2 = min(w, real_x + zoom_size)
        y2 = min(h, real_y + zoom_size)
        ...
        zh, zw = zoom.shape[:2]
        cv2.line(zoom, (zw // 2, 0), (zw // 2, zh), (0, 0, 255), 1)
        cv2.line(zoom, (0, zh // 2), (zw, zh // 2), (0, 0, 255), 1)

        cv2.imshow(self.zoom_window_name, zoom)
        cv2.resizeWindow(self.zoom_window_name, 250, 250)
